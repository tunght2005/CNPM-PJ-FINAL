  ...
  k.prototype = {
    ...
    _process_options: function (e) {
      ...
      var f = (this.o = a.extend({}, this._o)),
        g = f.language;
      switch (
        (q[g] || ((g = g.split("-")[0]), q[g] || (g = o.language)),
        (f.language = g),
        f.startView)
      ) {
        case 2:
        ...
        (f.startDate !== -(1 / 0) &&
          (f.startDate
            ? f.startDate instanceof Date
              ? (f.startDate = this._local_to_utc(this._zero_time(f.startDate)))
              : (f.startDate = r.parseDate(f.startDate, h, f.language))
            : (f.startDate = -(1 / 0))),
        f.endDate !== 1 / 0 &&
          (f.endDate
            ? f.endDate instanceof Date
              ? (f.endDate = this._local_to_utc(this._zero_time(f.endDate)))
              : (f.endDate = r.parseDate(f.endDate, h, f.language))
            : (f.endDate = 1 / 0)),
    ...
    fillMonths: function () {
      for (var a = "", b = 0; 12 > b; )
        a +=
          '<span class="month">' +
          q[this.o.language].monthsShort[b++] +
          "</span>";
    ...
    fill: function () {
      ...
      if (!isNaN(f) && !isNaN(g)) {
        this.picker
          ...
          .text(r.formatDate(new c(f, g), n, this.o.language)),
          this.picker
            .find("tfoot .today")
            .text(l)
        ...
        (this.each(function () {
          ...
          if (!f) {
            ...
            if (b.hasClass("input-daterange") || p.inputs) {
              var q = { inputs: p.inputs || b.find("input").toArray() };
      ...
      parseDate: function (d, e, f) {
        ...
        if (
          ...
          for (j = 0, v = u.length; v > j; j++) {
            if (((n = parseInt(m[j], 10)), (h = u[j]), isNaN(n)))
              switch (h) {
                case "MM":
          ...
          for (j = 0; j < s.length; j++)
            (x = s[j]),
              x in p &&
                !isNaN(p[x]) &&
                ((w = new Date(d)), t[x](w, p[x]), isNaN(w) || (d = w));
        }
        return d;
      },
